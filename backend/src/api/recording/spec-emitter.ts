 ...
 *
 * Portado de `backend-laravel/app/Support/Recording/SpecEmitter.php`, linha a linha: os casos de
 * borda daqui foram descobertos um a um, e reescrever com liberdade perderia os que não estão
 * óbvios no código.
 */
import { EnvKey } from '../project/environment/env-key.js'
import type { ActiveVars } from '../playwright/active-vars.js'
import { Playwright } from '../playwright/playwright.js'
import type { Url } from '../playwright/url.js'
import type { RecordedEvent, Selectors } from './events.js'
import { Recording, SENSITIVE_PREFIX } from './recording.js'
...
/** O que o `Str::slug($source, '_')` do Laravel faz: sem acento, sem pontuação, junto por `_`. */
...
/** O que o `Str::limit($source, 60, '')` faz: corta em 60 e apara o espaço que sobra na ponta. */
...
export class SpecEmitter {
    ...
    /**
     * O arquivo de login. Termina salvando a sessão no mesmo arquivo que o playwright.config lê, e
     * só depois de confirmar que o login aconteceu.
     */
    ...
    private entry(
        ...
        // O clique já marcou a caixa; o `fill` que vem atrás é o mesmo gesto contado duas vezes.
    ...
    /**
     * O clique no botão já enviou o formulário; sem clique antes, quem enviou foi o Enter no último
     * campo preenchido.
     */
    ...
    private names(): Record<string, string> {
        ...
        // O PHP ordena com SORT_NATURAL: SENSIVEL_2 vem antes de SENSIVEL_10.
    ...
    /**
     * O último segmento do caminho que serve para reconhecer a tela. Fora identificador, fora o que
     * a URL base já carrega: esperar por segmento que toda tela do sistema tem é esperar por nada, e
     * o teste segue antes da página trocar.
     */
    ...
    /**
     * O alvo entre aspas no título do passo: separa o que veio da tela do verbo que o descreve.
     * Substantivo genérico ("o campo", "o elemento") fica sem aspas de propósito — não é nome de
     * nada.
     */
