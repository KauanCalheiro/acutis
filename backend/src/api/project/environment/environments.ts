/**
 * Os ambientes de um projeto: cada um é um arquivo JSON em `environments/`, e um deles é o ativo.
 *
 * O ambiente ativo mora no `.env` do projeto, não aqui, porque é estado da máquina de quem está
 * usando — dois desenvolvedores no mesmo repositório apontam para ambientes diferentes sem
 * conflitar.
 */
import { existsSync, mkdirSync, readdirSync, readFileSync, rmSync, writeFileSync } from 'node:fs'
import { basename, join } from 'node:path'
import { authExists } from '../auth/auth.js'
...
export class Environments {
    ...
    resolve(): Record<string, string> {
        ...
        delete values[EnvKey.ACTIVE_ENVIRONMENT]
