/**
 * Um cenário do projeto: o par `tests/<id>.spec.ts` + `features/<id>.feature`.
 *
 * O id é o caminho do spec sem `tests/` e sem a extensão — é ele que aparece na URL, e é por isso
 * que um cenário dentro de subpasta (`checkout/pagar`) continua tendo um id só.
 */
...
/**
 * O spec gravado importa o wrapper do runner; servido para leitura, o import volta a ser o do
 * Playwright, que é o que o usuário reconhece.
 */
...
export function listScenarios(projectPath: string): ScenarioData[] {
    ...
    const specs = readdirSync(testsDir, { withFileTypes: true, recursive: true })
        ...
        // Só um nível de subpasta, como o glob do Laravel fazia.
