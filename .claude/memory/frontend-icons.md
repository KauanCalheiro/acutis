...
- `nuxt.config` → `icon.clientBundle.scan` embarca no bundle os ícones que o código usa. Sem isso o SSR do build publicado tenta buscar cada `ic:*` na rede e cospe `[Icon] failed to load icon` no terminal do CLI. Nome de ícone montado em runtime (template string, concatenação) escapa da varredura — sempre literal.
