...
Instruções persistentes ficam em `./.claude/memory/`. Antes de qualquer ação em um domínio, ler o arquivo correspondente — as regras vivem lá, não aqui. `backend.md`, `frontend.md`, `webdriver.md`, `e2e.md` e `structure.md` são **índices**: levam às sub-memórias específicas (ler o índice primeiro).
...
| [docker.md](docker.md) | feedback | subir serviço, rodar comando de dev/teste — tudo via docker compose, nunca no host |
