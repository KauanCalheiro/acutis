...
- **`columns` só com `accessorKey`** — nada de `header`/`cell` no objeto da coluna. Exceção: coluna de display sem dado (ex.: ações) usa `{ id: "acoes" }` (ver [[frontend-crud]]).
...
Fetch/paginação/filtros da fonte de dados: ver [[frontend-api]].
