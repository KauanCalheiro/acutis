...
- **`useDevice` + SSR**: componentes que escolhem variante por `useDevice` (ex.: `<Modal>` desktop/mobile via [[frontend-table]]... na verdade `components/modal`) DEVEM ficar dentro de `<ClientOnly>` — senão o SSR escolhe uma variante e o client outra → **hydration mismatch**.
