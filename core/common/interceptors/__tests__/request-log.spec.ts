...
import { logDir, withRequestLog, type RequestEntry } from '../request-log.interceptor.js'
...
function application() {
  ...
  router.post('/exemplo/chama', created(defineEventHandler(async (event) => {
  ...
  router.get('/exemplo/quebra', defineEventHandler(() => {
  ...
  router.post('/exemplo/stream', created(defineEventHandler(async () => {
    const response = await fetch(`${remoteUrl}/stream`, { method: 'POST', body: '{}' })

    return { texto: await response.text() }
  })))

  router.post('/exemplo/quieto', created(defineEventHandler(async (event) => {
    const body = await readBody<Record<string, unknown>>(event)
    return { recebido: Object.keys(body).length }
  })))

  /** Serviço que responde texto puro, não JSON. */
  router.post('/exemplo/texto', created(defineEventHandler(async () => {
  ...
  router.post('/exemplo/stream-sujo', created(defineEventHandler(async () => {
  ...
  router.post('/exemplo/fora-do-ar', created(defineEventHandler(async () => {
    try {
  ...
  const app = createApp()
  app.use(withRequestLog(router.handler))
  return app
}
