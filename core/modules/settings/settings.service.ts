...
import type { AiCredential } from './entities/ai-credential.entity.js'
...
import type { PingResponse } from '#shared/contracts/settings'
...
export class SettingsService {
  constructor(
    ...
    private readonly aiCredentials: Repository<AiCredential>
  ) {}
  ...
  async update(input: {
    ...
    if (provider !== '' && blankToNull(input.model) === null) {
      throw new ValidationFailed({ model: ['O modelo é obrigatório.'] })
    }
  ...
  async ping(input: { provider: string, key?: string | null, url?: string | null, model: string }): Promise<PingResponse> {
    this.ensureKnownProvider(input.provider)
  ...
  private ensureKnownProvider(provider: string): void {
    if (provider === '' || PROVIDER_NAMES.includes(provider)) return

    throw new ValidationFailed({ provider: ['O provedor informado não existe.'] })
  }
