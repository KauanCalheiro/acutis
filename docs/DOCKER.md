...
Frontend e backend num ambiente isolado e reproduzível, sem precisar de Node ou pnpm no host. Hot reload funciona nos dois. A alternativa direta no host está em [LOCAL.md](LOCAL.md).
...
docker compose -f docker-compose.dev.yml logs -f <serviço>   # backend | frontend
```

**Linux nativo (Docker Engine, sem Docker Desktop):** suba com o override, senão o backend não alcança o Chrome do host e a gravação não funciona (ver [Gravação](#gravação-chrome-do-host-via-cdp)):
...
| backend (NestJS) | 24000 | 4000 | http://localhost:24000 | API `/api/v1`, gravador e runner; precisa de WebSocket upgrade em `/ws` |

`NUXT_PUBLIC_WEBDRIVER_ACUTIS_URL` (frontend) e `CORS_ORIGIN` (backend) já vêm no compose apontando para as portas publicadas. Trocou porta ou domínio, ajuste as duas — o compose lê essas variáveis de um `.env` na raiz, se existir, e `environment` ganha de `env_file`.

## Rodar comando pontual

pnpm, testes: sempre via `exec`, **nunca no host** (o host pode nem ter o runtime, e o estado é o do container).
...
- **Instalou dependência? Recrie o serviço.** Os `node_modules` de frontend e backend vivem em volumes nomeados: rodar `pnpm add` no host **não** instala no container, e o serviço quebra com `Cannot find package`. Depois de qualquer mudança de dependência:
...
- **Linux nativo precisa do override por causa do Chrome.** Desde a ~v136 o Chrome ignora `--remote-debugging-address` e só aceita CDP em `127.0.0.1`; no Docker Engine nativo, `host.docker.internal` aponta para o IP da bridge, não para o loopback. O `docker-compose.linux.yml` resolve colocando o backend em `network_mode: host`, igualando o `127.0.0.1` dele ao da máquina. No macOS e no Windows (Docker Desktop) isso não é necessário: a VM já expõe o loopback do host.
- **Permissões nos bind mounts.** Cada `entrypoint.sh` detecta o UID/GID dono de `/app` e se reexecuta via `gosu` como usuário `dev`, para que `node_modules/`, `.nuxt/` e `dist-ui/` saiam com o dono do host em vez de root. Depende da imagem ter `gosu` — se editar o Dockerfile, rebuild.
