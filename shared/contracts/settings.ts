...
export const aiSettingsSchema = z.object({
  ...
  keyless_providers: z.array(z.string())
})
...
export const aiSettingsRequestSchema = z.object({
  ...
  url: z.string().nullable().optional().refine((url) => {
  ...
  }, 'A URL do provedor deve ser uma URL válida.'),
  ...
  if (data.provider && !data.model?.trim()) {
    context.addIssue({
      code: 'custom',
      path: ['model'],
      message: 'Escolha um modelo do provedor.'
    })
  }
})

export type AiSettingsRequest = z.output<typeof aiSettingsRequestSchema>
