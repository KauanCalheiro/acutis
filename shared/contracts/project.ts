...
export const createProjectSchema = z.object({
  name: z.string().trim()
    ...
    .refine(name => /[\p{L}\p{N}]/u.test(name), 'O nome deve conter ao menos um caractere alfanumérico.')
})

export type CreateProjectRequest = z.output<typeof createProjectSchema>

export const cloneProjectSchema = z.object({
  url: z.string().trim().min(1, 'A URL do repositório é obrigatória.'),
  name: z.string().trim().max(255, 'O nome não pode ter mais de 255 caracteres.').optional(),
  branch: z.string().trim().max(255, 'A branch não pode ter mais de 255 caracteres.').optional(),
  auth: z.enum(['public', 'token', 'ssh_key']).default('public'),
...
export const projectDetailSchema = projectSchema.extend({
  ...
  auth_status: z.enum(['unset', 'skipped', 'configured', 'failing']),
