...
export type ScenarioFixBffRequest = ScenarioFixRequest & { scenarioId: string }
export type ScenarioSuggestionsRequest = { scenarioId: string }
...
export type RunTimelineStep = {
    title: string; status: 'waiting' | 'running' | 'success' | 'failed'; error?: string | null; testId?: string; order?: number
}
export type RunTest = { id: string; title: string; status: RunTimelineStep['status']; error: string | null; steps: RunTimelineStep[] }
export type RunStreamEvent = {
    event: 'run:started' | 'step' | 'test' | 'run:finished'; id?: string; testId?: string; title: string;
    status: 'pending' | 'success' | 'failed' | 'skipped'; steps?: string[]; error?: string | null;
    videoPath?: string | null; passed: boolean; output?: string
}
